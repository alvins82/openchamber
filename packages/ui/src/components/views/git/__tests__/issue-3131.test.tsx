...
// Issue 3131: the PR merge flow preselects `squash` without any user choice and
// merges on a single click next to the method selector, with no confirmation.
// These tests reproduce that behavior against the current source.
...
describe('issue 3131: PR merge defaults to squash and merges on a single click', () => {
  beforeEach(() => {
    for (const key of Object.keys(mockEntryState)) {
  ...
  test('opens the panel with squash pre-selected and an enabled merge button', () => {
    seedMergeableStatus();

    const html = renderPanel();

    // The method selector is pre-filled with the raw squash value before the
    // user has touched anything.
    expect(html).toContain('value="squash"');
    // The icon-only merge button sits right next to the selector and is
    // rendered enabled for an open, mergeable PR.
    expect(html).toContain('aria-label="Merge pull request"');
    // No confirmation dialog exists anywhere in the panel.
    expect(html).not.toContain('role="dialog"');
  });

  test('traces the source: mergeMethod is hard-coded to squash', () => {
    expect(pullRequestSectionSource).toContain(
      "const [mergeMethod, setMergeMethod] = React.useState<MergeMethod>('squash');"
    );
  });

  test('traces the source: the merge button merges immediately on click', () => {
    // Single click on the icon-only button calls mergePr directly; there is no
    // confirmation state or dialog in between.
    expect(pullRequestSectionSource).toContain('onClick={() => mergePr(pr)}');
    // The button's disabled condition never consults the selected method, so a
    // pre-selected squash stays actionable for an open, non-draft PR.
    expect(pullRequestSectionSource).toContain(
      "disabled={isMerging || isMarkingReady || pr.state !== 'open' || pr.draft || isUpdating || isEditingPr}"
    );
    expect(pullRequestSectionSource).not.toContain('if (!mergeMethod)');
    // mergePr sends the currently selected method straight to the API.
    expect(pullRequestSectionSource).toContain(
      'const result = await github.prMerge({ directory, number: pr.number, method: mergeMethod });'
    );
  });

  test('traces the source: client always sends squash while the server falls back to merge', () => {
    // The UI hard-codes squash (asserted above); the server defaults the
    // method to merge when it is absent, so the two defaults disagree.
