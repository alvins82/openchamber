...
describe('issue 2876: shell widget copy -> integrated terminal paste', () => {
    ...
    test('the widget copy handler puts the raw control sequences on the clipboard verbatim', async () => {
        ...
        const result = await copyTextToClipboard(RAW_SPINNER_OUTPUT);

        expect(result.ok).toBe(true);
        expect(written).toBe(RAW_SPINNER_OUTPUT);
    });

    test('pasting that clipboard content into the integrated terminal collapses to a single character', () => {
        // Content on the clipboard after pressing the shell widget copy button.
        const clipboardText = RAW_SPINNER_OUTPUT;
        ...
        // Nearly all of the copied text is erased by the \r / ESC [ 2 K
        // sequences; only the final spinner frame (a single character) remains.
        expect(visibleAfterPaste.trim()).toBe('\u2713');
    });

    test('progress-style output pastes truncated to its last frame', () => {
