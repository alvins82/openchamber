...
Terminal session failures are kept in the live `session-failure-store.ts` alongside the event stream rather than inferred from idle status. A `session.error` or `session.next.step.failed` event records the failure for the session's directory; a later busy event clears it for a new attempt, and deletion clears it with the session. Idle status alone never turns a failed session into a successful one.
