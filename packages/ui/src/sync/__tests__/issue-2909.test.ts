...
describe("issue-2909 — post-rollover prompt send is rolled back although the server accepted it", () => {
  ...
  test("current behavior: the accepted prompt IS rolled back (documents the bug)", async () => {
    const sessionID = "session-rollover"
    const directory = "/test/project"
    const existing = buildPreRolloverMessages(30, sessionID)
    ...
    // The buggy behavior on current main: the accepted prompt is rolled back
    // and the send rethrows, so the user's message disappears from the chat.
    expect(caught).toBeInstanceOf(Error)
    expect(optimisticRemoveCalled).toBe(true)
    expect(store.getState().message[sessionID].some((m) => m.id === sentMessageID)).toBe(false)
    expect(store.getState().session_status[sessionID]?.type).toBe("idle")
  })
})
